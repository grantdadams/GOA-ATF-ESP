#include <TMB.hpp>
// UPDATED with arrowtooth assessment log-likelihoods
// ------------------------------------------------------------------------- //
...
Type objective_function<Type>::operator() () {
      ...
      if(flt_type(flt) == 1){
        if(flt_yr <= endyr){
          if(fsh_biom_obs(fsh_ind, 0) * fsh_bio_hat(fsh_ind) > 0){ //FIXME: dont like the fsh_bio_hat bit
            ...
            // ATFs
            jnll_comp(1, flt) += square((log(fsh_biom_obs(fsh_ind, 0) + 0.000001) - log(fsh_bio_hat(fsh_ind) + 0.000001))) * (0.5/square(fsh_std_dev));
        ...
        if(yr <= endyr){
          for (ln = 0; ln < n_comp; ln++) {
            if(!isNA( comp_obs(comp_ind, ln) )){
                // Martin's
                jnll_comp(2, flt) -= comp_weights(flt) * Type(comp_n(comp_ind, 1)) * (comp_obs(comp_ind, ln) + 0.000001) * log((comp_hat(comp_ind, ln)+0.000001) / (comp_obs(comp_ind, ln) + 0.000001)) ;
            }
          }
        }
      }
      ...
      if (flt_sel_type(flt) == 2) {
        ...
        for(sex = 0; sex < nsex(sp); sex++){
          for(age = 0; age < (nages(sp) - 1); age++) {
            if( sel(flt, sex, age, 0) > sel(flt, sex, age + 1, 0)) {
              jnll_comp(4, flt) += sel_curve_pen(flt, 0) * square( log(sel(flt, sex, age, 0) / sel(flt, sex, age + 1, 0) ));
            }
          }
        }
        ...
        for(sex = 0; sex < nsex(sp); sex++){
          ...
          for (age = 0; age < nages(sp) - 2; age++) {
            ...
            jnll_comp(4, flt) += sel_curve_pen(flt, 1) * square( sel_tmp(age));
          }
        }
      }
    }
  ...
  for(flt = 0; flt < n_flt; flt++){
    ...
    for(sex = 0; sex < nsex(sp); sex++){
      if(flt_type(flt) > 0){
        jnll_comp(6, flt) += 1.0 * square(avgsel(flt, sex));
      }
    }
  }
